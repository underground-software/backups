...
cp -a $PREFIX/cano.py/venus/users.db $SCRIPT_DIR/users.db.$TIMESTAMP

echo "[2]: back up grades.db from mercury"

cp -a $PREFIX/cano.py/mercury/grades.db $SCRIPT_DIR/grades.db.$TIMESTAMP

echo "[3]: back up email_data"

DEST=$PREFIX/backups/email_data.$TIMESTAMP
mkdir -p $DEST
cp -ar $PREFIX/email_data $DEST
